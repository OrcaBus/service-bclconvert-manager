...
def mock_generate_db_uuid():
    return {"db_uuid": "12345678-1234-5678-1234-567812345678"}
...
class TestICAv2EventTranslator(unittest.TestCase):
    def setUp(self):
        self.test_event = {
            ...
            "detail": {
                "ica-event": {
                    ...
                    "eventParameters": {"analysisStatus": "UNSPECIFIED"},
                    "payload": {
                        "id": "valid_payload_id",
                        "userReference": "123456_A1234_0000_TestingPattern",
                        "timeCreated": "2024-01-01T00:11:35Z",
        ...
        self.events = botocore.session.get_session().create_client(
            "events", region_name="ap-southeast-2"
        )
        self.dynamodb = botocore.session.get_session().create_client(
        ...
        patches = [
            ...
            patch(
                "icav2_event_translator.generate_db_uuid",
                return_value=mock_generate_db_uuid(),
            ),
            patch(
                "icav2_event_translator.generate_portal_run_id",
                return_value="2024010112345678",
            ),
            patch("icav2_event_translator.set_icav2_env_vars", return_value=None),
            patch(
                "icav2_event_translator.collect_analysis_objects",
                return_value=mock_collect_analysis_objects(),
            ),
        ]
        for p in patches:
            p.start()
    ...
    def setup_event(self, status):
        ...
        """
        self.test_event["detail"]["ica-event"]["eventParameters"][
            "analysisStatus"
        ] = status
    ...
    def run_event_test(self):
        ...
        expected_item = {
            ...
            "original_external_event": {
                "S": json.dumps(self.test_event["detail"]["ica-event"])
            },
            "translated_internal_ica_event": {
                "S": json.dumps(expected_ica_event_details)
            },
            "timestamp": {"S": expected_ica_event_details["timestamp"]},
        }
        expected_params = {"Item": expected_item, "TableName": "test_table"}
        self.dynamodb_stubber.add_response("put_item", response, expected_params)

        expected_params = {
            "TableName": "test_table",
            "Key": {"id": {"S": "valid_payload_id"}, "id_type": {"S": "analysis_id"}},
        ...
        expected_params = {
            "TableName": "test_table",
            "Key": {"id": {"S": "2024010112345678"}, "id_type": {"S": "portal_run_id"}},
            "UpdateExpression": "SET db_uuid = :db_uuid",
            "ExpressionAttributeValues": {
                ":db_uuid": {"S": "12345678-1234-5678-1234-567812345678"}
            },
